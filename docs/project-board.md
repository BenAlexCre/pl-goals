...
*(nothing ready — Slice 9 implemented and verified, awaiting review/approval before Milestone 5 (LMS) begins)*

## In Progress

*(nothing in progress — Slice 9 implemented and verified, awaiting review/approval before Milestone 5 (LMS) begins)*
...
- **Milestone 4, Slice 9 — Notifications (domain events)** → [game-engine.md § GE-12](./game-engine.md#ge-12-milestone-plan). `Pick5Engine.notifyUsers()` (the eighth and final real Game Engine contract implementation for Pick 5 — all eight `GameEngine` methods now implemented) — a pure domain-event emitter, inserting one row into `notifications` and returning; never calls or knows about a delivery channel. Called from within `awardPrize()`, once per winner, after that winner's `payout_amount` is durably written — see [decisions.md § Notifications: domain events, not delivery](./decisions.md#notifications-domain-events-not-delivery) for the full design review (event catalog: only `pick5.prize_awarded` implemented, other candidates deliberately deferred; why a notification failure is caught and logged rather than allowed to affect settlement; why no retry logic belongs at this layer; how this extends to LMS/Predictor with no new schema). Resolves no `ISSUE-N` (net-new Game Engine build-out). 7 new Deno unit tests (59/59 in file, 87/87 total pass). Verified live end-to-end through the real `settle-gameweek` Edge Function: a winner's `pick5.prize_awarded` notification written with the correct `payload` (`gameweekId`, `amount`), a second idempotent invocation did not duplicate it. All test data removed by exact ID. **Not yet committed** — awaiting review/approval (see `session-log.md`).
