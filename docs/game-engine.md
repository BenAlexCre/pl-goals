...
**Milestone 6 Slice 6 — corrected, 2026-08-08**: `LmsEngine.generateStandings()`'s
own `game_entries` query had no `status` filter at all, so a voided
(unpaid) entry — whose `game_entry_lms.competitive_status` `settle()`'s
void step never touches — could still render, most often in the "alive"
tier. Fixed with `.neq('status', 'void')`. Full reasoning:
[decisions.md § LMS standings must exclude voided entries](./decisions.md#lms-standings-must-exclude-voided-entries).
**`PredictorEngine.generateStandings()`** is the third writer of this
table, also overall-row-only like LMS (no per-gameweek payout concept for
Predictor either — GE-5.3), but ranked like Pick 5 (`total_points` is a
real, comparable score, unlike LMS's synthetic alive/eliminated
indicator) — reuses Pick 5's exact `rankWithTies()` algorithm, duplicated
per GE-18 rather than imported. `meta`: `{ exactScoreCount,
correctScorerCount }`. Same partial-index upsert workaround, a third
private copy. Full reasoning:
...
**Standings — implemented, Milestone 6 Slice 6, 2026-08-08, split down the
middle between Pick 5 and LMS, not modeled on either wholesale.**
`PredictorEngine.generateStandings()` ranks every non-void entry by
`game_entry_predictor.total_points` using Pick 5's own standard
competition ranking (`rankWithTies()`, duplicated per GE-18) — a genuine
score, unlike LMS's synthetic alive/eliminated indicator — but writes
only the overall row (`gameweek_id = null`), like LMS, since Predictor
has no per-gameweek payout concept the way Pick 5's weekly jackpot does.
`meta`: `{ exactScoreCount, correctScorerCount }`. Void entries excluded
via `.neq('status', 'void')` — the same corrected filter just applied to
`LmsEngine.generateStandings()` (below). `settle()` was restructured, not
just appended to — its "nobody's unpaid" early return would otherwise
have skipped standings on the overwhelmingly common tick, so the
payment-void logic now sits inside a conditional block and
`generateStandings()` runs unconditionally per eligible pot afterward,
the same revision Pick5Engine's/LmsEngine's own `settle()` needed when
each of their own Slice 6 shipped generateStandings() for the first time.
Full reasoning, including all eight architecture-review questions
resolved before coding:
[decisions.md § Score Predictor standings](./decisions.md#score-predictor-standings).

**Winner determination — implemented, Milestone 6 Slice 7, 2026-08-08,
not modeled on either existing mode wholesale.**
`PredictorEngine.determineWinner()` recomputes directly from
`game_entries`/`game_entry_predictor` — never `pot_standings_snapshots` —
matching `LmsEngine`'s own choice (a cache can go stale;
`Pick5Engine.determineWinner()`'s own snapshot read is only safe because a
settled Pick 5 gameweek can never change retroactively), and this slice's
own explicit "never depend on cached state" requirement. Returns every
non-void entry tied for the highest cumulative `total_points` once the
pot's `end_gameweek_id` deadline has passed — Predictor's *only*
conclusion path, unlike LMS's four (single survivor/wipeout/season-end/
in-progress), since no elimination concept exists to conclude the
competition early. `predictor_cycle_mode` confirmed, not assumed, to have
no bearing at all: GE-6's fixed `(ctx, potId)` signature can only ever
answer the season-end question, and that computation doesn't change based
on cycle mode — the still-open "does `two_halves` need a *separate*,
earlier determination" question (GE-15) remains exactly as unresolved as
before, neither guessed at nor blocked on. A private `classifyOutcome()`
helper (mirroring `LmsEngine`'s own split) is reusable by a future
`awardPrize()` slice. Full reasoning, including all eight
architecture-review questions resolved before coding:
[decisions.md § Score Predictor winner determination](./decisions.md#score-predictor-winner-determination).

**Tiebreak revised, Milestone 6 Slice 8, 2026-08-08/09** — a real product
rule arrived stated as a Pick 5 change, but its vocabulary ("exact score
predictions," "correct goalscorer predictions") matched nothing in Pick
5's own pick model, only Predictor's `game_entry_predictor.exact_score_count`/
`correct_scorer_count`; confirmed with the repo owner directly rather
than guessed at (per the task's own "stop and ask" instruction) — the
rule was meant for Predictor, and Pick 5's own `determineWinner()`/
`awardPrize()` are unchanged. `classifyOutcome()`'s `season_end` winner
set now narrows through three levels when the level above is tied:
highest `total_points`, then highest `exact_score_count`, then highest
`correct_scorer_count`; whatever remains genuinely ties and splits
equally, unchanged mechanism. Deliberately not applied to
`generateStandings()`'s own ranking. Full reasoning:
[decisions.md § Score Predictor prize awarding](./decisions.md#score-predictor-prize-awarding).

**Prize awarding — implemented, Milestone 6 Slice 8, 2026-08-08/09.**
`PredictorEngine.awardPrize()` calls `determineWinner()` directly (no
richer type to lose, unlike `LmsEngine`'s own `classifyOutcome()`-direct
choice — matches `Pick5Engine.awardPrize()`'s reasoning instead). One
`pot_prizes` row, `scope='season'`, matching LMS's shape (one conclusion,
ever, per pot) not Pick 5's weekly multi-row shape. Deductions identical
to Pick 5/LMS (`admin_fee_*`/`charity_fee_*`, shared `pots` columns,
private duplicate math per GE-18) — `gross_amount` has no
`carry_over_amount` term, an LMS-rollover-only concept with no Predictor
equivalent. Zero winners is a silent no-op, matching `LmsEngine`'s
philosophy (not `Pick5Engine`'s throw) — "not concluded yet" is
Predictor's normal, common state. `pot_prizes` written last, applying the
hardening-sprint's write-ordering lesson from the start rather than
retrofitting it. New error class, `PredictorPrizePoolExceededError`,
mirrors both existing modes' own. `settle()`'s per-pot loop now also
calls `awardPrize()`, the same revision Pick5's/LMS's own `settle()`
needed at this same point. No schema change. Full reasoning, including
all seven architecture-review questions resolved before coding:
[decisions.md § Score Predictor prize awarding](./decisions.md#score-predictor-prize-awarding).

**Notifications — implemented, Milestone 6 Slice 9, 2026-08-09.**
`PredictorEngine.notifyUsers()` is a near-identical duplicate of
`Pick5Engine.notifyUsers()`/`LmsEngine.notifyUsers()` — justified because
those two existing implementations are themselves byte-for-byte identical
to each other, leaving no genuine design-decision surface for a third copy
to diverge on: insert one `notifications` row, throw on error. New event
type `predictor.prize_awarded` (only one event exists, mirroring both
existing modes' own single event — Predictor's `awardPrize()` has exactly
one non-trivial outcome shape, `season_end`, so there is nothing else for
a notification to describe). Payload is `{ amount, tied }` —
`tied: winners.length > 1` is the Predictor-specific analog to Pick5's
`gameweekId`/LMS's `outcome`, chosen because neither of those existing
fields would carry real information for Predictor's single-outcome,
non-gameweek-scoped shape. One notification per winning user, called from
inside `awardPrize()`'s recipient loop, immediately after the trailing
`pot_prizes` write, each call wrapped in its own try/catch that logs and
never propagates — settlement and payout are already durably written by
the time a notification is even attempted, so a write failure can never
undo or block the prize. No new dedup mechanism needed: idempotency comes
free from `awardPrize()`'s own pre-existing `pot_prizes.is_settled`
short-circuit, which prevents the notify loop from ever being reached
again once a pot is fully awarded. No schema change. All eight GameEngine
contract methods are now implemented for Score Predictor. Full reasoning,
including all six architecture-review questions resolved before coding:
[decisions.md § Score Predictor notifications](./decisions.md#score-predictor-notifications).
