...
Deno.serve(async (req) => {
  ...
  try {
    switch (action) {
      ...
      case 'mark_unpaid': {
        // Launch Readiness Sprint 1B: this used to also write
        // `is_void: true`/`status: 'void'` to `user_entries` — the retired
        // prototype table (pre-Game-Engine schema), not `game_entries`.
        // That table still physically exists (ISSUE-20) but no current
        // code reads from it, and `.eq('gameweek_id', gameweek_id)` with a
        // null gameweek_id (every LMS/Predictor call) can never match any
        // row in SQL regardless — the update was already a confirmed
        // no-op for every mode, every time. Settlement never depended on
        // it either: Pick5Engine.settle() (and LMS's/Predictor's own
        // settle()) read `entry_payments.is_paid` directly at settlement
        // time, not a pre-set void flag. Removed as dead code, not a
        // behavior change — confirmed via source read before removing,
        // not assumed.
        const { user_id, gameweek_id } = body
