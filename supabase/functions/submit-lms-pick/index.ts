...
import { LmsValidationError } from '../_shared/game-engine/lms/index.ts'
import type { GameEntry } from '../_shared/game-engine/types.ts'
import { validateSubmitLmsPickRequest } from './validate.ts'

// Milestone 5, Slice 2 (pick submission) — docs/game-engine.md § GE-5.2 / GE-6.
// Mirrors submit-pick5-picks/index.ts closely; the real difference is
// LMS's shape (one team per gameweek, not five players per gameweek-scoped
// entry) — a season-scoped game_entries row has no gameweek_id of its own,
// so gameweek_id is a request parameter here, not read off the entry.
//
// Auth pattern identical to submit-pick5-picks/get-or-create-lms-entry: a
...
// lms_team_picks has no client-insert RLS policy at all (015_lms_picks.sql,
// named lms_team_picks not lms_picks to avoid colliding with the retired
// prototype's own table of that name), so this function is the only path
// that can write one.
...
Deno.serve(async (req) => {
  ...
  const { gameEntryId, gameweekId, teamId } = validation
  ...
  if (entryRow.pots?.game_type !== 'last_man_standing') {
    return jsonResponse({ error: `This entry belongs to a ${entryRow.pots?.game_type} pot, not last_man_standing` }, 400)
  }
  ...
  try {
    await resolveEngine('last_man_standing').validateEntry(ctx, entry, { gameweekId, teamId })
  } catch (err) {
    if (err instanceof LmsValidationError) {
      return jsonResponse({ error: err.message }, 400)
    }
    return jsonResponse({ error: err instanceof Error ? err.message : 'Validation failed' }, 500)
  }

  // Same re-check-immediately-before-the-write pattern as submit-pick5-picks
  // (docs/decisions.md § Same-request write races) — validateEntry() above
  // checked entry.status against the snapshot read at the top of this
  // request; re-verified directly, immediately before the write, to close
  // the window as tightly as this request can.
  ...
  const { data: savedPick, error: upsertError } = await adminClient
    .from('lms_team_picks')
    .upsert(
      { game_entry_id: gameEntryId, gameweek_id: gameweekId, team_id: teamId },
