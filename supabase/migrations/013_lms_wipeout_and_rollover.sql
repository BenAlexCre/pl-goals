-- DESIGN ONLY — NOT APPLIED. Drafted for review ahead of Milestone 5 Slice 2,
-- per the repo owner's explicit "design it, review it, do not apply it"
-- instruction (same pattern as 010_prize_pool_deductions.sql). Supersedes an
-- earlier same-name-series draft (013_lms_tie_outcome_and_rollover.sql,
-- never applied, never committed) whose payment-model assumption the repo
-- owner's revised product decisions (2026-08-05) explicitly overturned —
-- see docs/decisions.md § LMS: Wipeout Resolution, automatic rollover, and
-- a fixed per-competition entry fee for the full reasoning, including what
-- changed from the first draft and why.
--
-- Four additions, all GE-3 platform/mode-boundary-respecting (config on
...
--      hand. carry_over_amount is the old pot's unclaimed net_amount,
--      copied onto the new pot explicitly (never re-derived later from the
--      old pot_prizes row, so the new pot's own prize-pool math never needs
--      a cross-pot join at settlement time).
--
...
comment on column public.pots.rollover_source_pot_id is
  'GE-5.2: set only by the Game Engine itself, automatically, when wipeout_resolution = roll_prize resolves with no winner — never by an organiser, never through the normal pot-creation path. Unconditionally immutable, like game_type — this is the pot''s lineage. NULL for every pot that isn''t a Game-Engine-created rollover.';
comment on column public.pots.carry_over_amount is
  'GE-5.2: the source pot''s unclaimed net_amount (see pot_prizes.rollover), copied explicitly onto this pot at automatic-creation time. Added to this competition''s own gross prize pool once it settles. Zero, and meaningless, unless rollover_source_pot_id is set. Unconditionally immutable, same set as rollover_source_pot_id.';
...
-- rollover_source_pot_id/carry_over_amount join game_type's unconditional
-- set instead — both are set exactly once, atomically, by the Game Engine
-- at the moment it creates the pot, never editable by anyone afterward,
-- entries or not. create or replace, not a rewrite of the migration that
-- first created this function.
...
begin
  ...
  then
    raise exception 'pots.rollover_source_pot_id and pots.carry_over_amount cannot be changed after creation (GE-5.2)';
  end if;
